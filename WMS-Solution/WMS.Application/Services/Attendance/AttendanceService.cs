...
public class AttendanceService : IAttendanceService
{
    ...
    {
        attendanceDto.AttendanceDate = DateTime.Today;

        var existingAttendance =
            await _attendanceRepository.GetAttendanceByEmployeeAndDate(
    ...
    public async Task<IEnumerable<AttendanceDto>> GetAttendanceByEmployeeId(int employeeId)
    {
        var attendance =  await _attendanceRepository.GetAttendanceByEmployeeId(employeeId);

        return _mapper.Map<IEnumerable<AttendanceDto>>(attendance);
    }

    public async Task<IEnumerable<AttendanceDto>>GetAttendanceByMonth(
            int employeeId,
            int month,
            int year)
    {
        var attendance = await _attendanceRepository.GetAttendanceByMonth(
                employeeId,
                month,
                year
            );
